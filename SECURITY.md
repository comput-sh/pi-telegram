...
- Never commit `.pi/pi-telegram-extension.local.json`, global extension settings, bot tokens, private keys, or environment files.
- Global manager credentials are stored in `~/.pi/agent/pi-telegram-extension/settings.json`.
- Project bot credentials are stored in `<project>/.pi/pi-telegram-extension.local.json`.
